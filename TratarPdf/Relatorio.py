...
class Relatorio:
    ...
    def Salvar_Relatorio(self,save_path=None):
        ...
        for ficha in lista_de_fichas_filtrada:
            try:
                ...
                # if ficha.poste != None:
                #     print(f"{ficha.identificador} | {ficha.poste}")
                # self.Salvar_arquivo(ficha,save_path)
            except:
                print(f"{nome_arquivo}: {ficha}")


            # if len(ficha.parque) != 6:
                # self.print(f"{ficha} pág {ficha.paginas}")
    ...
    def Criar_dataframe(self, lista_de_fichas):
        dados = [{'Identificador': ficha.identificador,'Páginas':ficha.paginas, 'Parque': ficha.parque, 'Poste': ficha.poste, 'Tipo': ficha.tipo,'Setor': ficha.setor} for ficha in lista_de_fichas]
    ...
    def print(self,texto):
        path = os.path.dirname(self.caminho_pdf)
        nome_arquivo = os.path.basename(self.caminho_pdf) + " - "
        path_print = path + "/resultado2.txt"

        # Abrir o arquivo em modo de escrita

        with open(path_print, "a") as arquivo:
            # Escrever o texto no arquivo
            print(f"{nome_arquivo} {texto}", file=arquivo)
        ...
